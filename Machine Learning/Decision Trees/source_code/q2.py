...
def kfold_cv(model, xFeat, y, k):
    ...
    for train_index, test_index in k_fold.split(xFeat):
...
def mc_cv(model, xFeat, y, testSize, s):
    ...
    for i in range(s):
        ...
        xTrain, xTest, yTrain, yTest = train_test_split(xFeat_copy, y_copy, test_size=testSize, random_state=0)
        trainAuc, testAuc = sktree_train_test(model, xTrain, yTrain, xTest, yTest)
...
def sktree_train_test(model, xTrain, yTrain, xTest, yTest):
    ...
    model.fit(np.array(xTrain), np.array(yTrain).T[0])
