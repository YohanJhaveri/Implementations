...
def standard_scale(xTrain, xTest):
    ...
    for dataset in [xTrain, xTest]:
        ...
        for x in dataset:
            mean = np.mean(dataset[x])
            std = np.std(dataset[x])
            dataset[x] = dataset[x].apply(lambda x: (x - mean) / std)
...
def minmax_range(xTrain, xTest):
    ...
    for dataset in [xTrain, xTest]:
        ...
        for x in dataset:
            min = np.min(dataset[x])
            max = np.max(dataset[x])
            dataset[x] = dataset[x].apply(lambda x: (x * (1 / (max - min))) -  (min * (1 / (max - min))))
...
def add_irr_feature(xTrain, xTest):
    ...
    """
    xTrain['noise1'] = np.random.normal(0, 1, len(xTrain))
    xTrain['noise2'] = np.random.normal(0, 1, len(xTrain))
    xTest['noise1'] = np.random.normal(0, 1, len(xTest))
    xTest['noise2'] = np.random.normal(0, 1, len(xTest))
...
def main():
    ...
    xTrainIrr, xTrainIrr = add_irr_feature(xTrain, xTest)
    acc4 = knn_train_test(args.k, xTrainIrr, yTrain, xTrainIrr, yTest)
